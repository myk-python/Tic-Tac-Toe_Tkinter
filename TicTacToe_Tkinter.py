...
#         Creating these lists here so that we only need to do it once and
#         are free to access them from each X or O.
...
#         Used to explictly state the rows/columns to use - allows a condensed/more resource-friendly
#         for loop structure.
#         I.e. 3 rows * 3 columns + 9 buttons vs 3 rows * 3 columns * 9 buttons.
        self.rows_cols =[]
#         Keeping this outside of the buttoncreation function so that the grid is only created instantiation
#         of the main class; it doesn't have to be reloaded when the restart function runs.
#         Avoiding row 0 by convention, in case a title bar is added later.
    ...
    def restart(self):
#         Resets clickcount whilst maintaining the turn cycle.
#         Very unlikely to be an issue, but in theory enough
#         games (millions) could have caused clickcount memory errors.
...
#         Each X or O is a custom gif.
#         When clicked, the buttons disable and change to an 'active' colour.
...
#         Need to retrieve the position information of each X or O
#         prior to analysing it.
...
#         Appending row and column data because it is needed in order to change the image
#         when a diagonal win occurs.
...
#         Obviously cannot win without at least 3 X's or O's.
...
#         Changes the image of winning X's or O's.     
        ...
        try:
            if zwin ==True:
#                 Disables all buttons that aren't 'active'.
#                 I.e. aren't X's or O's.
                self.blocking()
        except:
#             3 rows + 3 columns.
